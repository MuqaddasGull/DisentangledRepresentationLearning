...
class CLASSIFIER:
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, opt, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, generalized=True):
        ...
        #print("self.train_feature",self.train_feature.shape)  [61598, 4096]
        self.train_label = data_loader.train_label
        #print("self.train_label",self.train_label.shape)      [61598, 65]
        
        self.test_seen_unseen_feature = data_loader.test_seen_unseen_feature
        #print("self.test_seen_unseen_feature",self.test_seen_unseen_feature.shape)   [10098, 4096]
        self.test_seen_unseen_label = data_loader.test_seen_unseen_label
        #print("self.test_seen_unseen_label",self.test_seen_unseen_label.shape)       [10098, 80]
        
        self.test_unseen_feature = data_loader.test_unseen_feature
        #print("self.test_unseen_feature",self.test_unseen_feature.shape)         [10098, 4096]
        self.test_unseen_label = data_loader.test_unseen_label
        #print("self.test_unseen_label",self.test_unseen_label.shape)              [10098, 15]
        
        self.seenclasses = data_loader.seenclasses
        #print("self.seenclasses",self.seenclasses.shape)          [65]
        self.unseenclasses = data_loader.unseenclasses
        #print("self.unseenclasses",self.unseenclasses.shape)        [15]
        ...
        if self.cuda:
            ...
            #self.train_feature=self.train_feature.cuda()
    ...
    def fit(self):
        ...
        for epoch in range(self.nepoch):
            for i in range(0, self.ntrain, self.batch_size):
                ...
                #print("self.input_fit",self.input.shape)    (300,4096)
                self.label.copy_(batch_label)
                #print("self.label_fit",self.label[20])     #(300,80)
                output = self.model(self.input)
                #print("output_fit1",output[20])             #(300,80)
                #print("output_fit2",torch.sigmoid(output[20]))
                #torch.sigmoid(input)
    ...
    def fit_zsl(self):
        ...
        for epoch in range(self.nepoch):
            for i in range(0, self.ntrain, self.batch_size):
                ...
                batch_input, batch_label = self.next_batch(self.batch_size)
                self.input.copy_(batch_input)
                #print("self.input_fit_zsl",self.input.shape)   (300,4096)
                self.label.copy_(batch_label)
                #print("self.label_fit_zsl",self.label.shape)    (300,15)
                output = self.model(self.input)
                #print("output_fir_zsl",output.shape)            (300,15)
                loss = F.binary_cross_entropy_with_logits(output, self.label.float()).cuda()
                loss.backward()
                self.optimizer.step()
...
class LINEAR_LOGSOFTMAX(nn.Module):
    def __init__(self, input_dim, nclass):
        ...
        self.fc1 = nn.Linear(input_dim, nclass) #(4096,81)
    def forward(self, x):
        o = self.fc1(x)
        return o
...
def EE(netE, xx):
    means_img, log_var_img = netE(x=xx,att=None)   
