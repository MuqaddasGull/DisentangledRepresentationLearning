...
class DATA_LOADER(object):
    ...
    def read_matdataset(self, opt):
        tic = time.time()
        src = "/muqadas/deeplearning/torch/Generative_MLZSL/datasets/NUS-WIDE" #folder for path containing features
        ...
        self.seen_cls_idx, unseen_cls_idx = get_seen_unseen_classes(file_tag1k, file_tag81)
        src_att = pickle.load(open(att_path, 'rb'))
        #print("src_att",src_att[0].shape)   #(1000,300)
        #print("src_att",src_att[1].shape)   #(81,300)
        print("attributes are combined in this order-> seen+unseen")
        
        #print ("src_att[self.seen_cls_idx]", src_att[0][0])
        ...
        self.seenclass_attribute= src_att[0][self.seen_cls_idx]
        self.unseenclass_attribute=src_att[1]
        
        #VGG features path
        #import pdb;pdb.set_trace()
        train_loc = load_dict_from_hdf5(os.path.join(src, 'nus_wide_vgg_features','nus_seen_train_vgg19_new.h5'))
        test_unseen_loc = load_dict_from_hdf5(os.path.join(src, 'nus_wide_vgg_features', 'nus_zsl_test_vgg19_new.h5'))
        test_seen_unseen_loc = load_dict_from_hdf5(os.path.join(src, 'nus_wide_vgg_features', 'nus_gzsl_test_vgg19_new.h5'))
        ...
        #print("label_train_loc",label_train_loc[0])    #(161789,925)
        ...
        #print("temp_label",temp_label.shape)   [161789, 925]
        temp_seen_labels = temp_label.sum(1)
        temp_label = temp_label[temp_seen_labels>0]  # remove those examples where no labels exist.
        #print("temp_label",temp_label.shape)   ([154812, 925]

        self.train_label           = temp_label     #[154812, 925]
        ...
        print("Data with N={} labels={}".format(opt.N,self.train_trimmed_label.shape)) #([141354, 925])
        print("Full Data labels={} with min label/feature = {} and max label/feature = {}".format(self.train_label.shape, temp_label.sum(1).min(), temp_label.sum(1).max()))  #min label/feature = 1 and max label/feature = 117
        ...
        #print("self.syn_num",self.syn_num)
        self.per_seen = opt.per_seen
        #print("self.per_seen",self.per_seen)
        self.per_unseen = opt.per_unseen
        #print("self.per_unseen",self.per_unseen)
        self.per_seen_unseen = opt.per_seen_unseen
        #print("self.per_seen_unseen",self.per_seen_unseen)

        print("USING TRAIN FEATURES WITH <=N")
        self.ntrain = self.train_trimmed_feature.size()[0]
        train_labels = self.train_trimmed_label  #([141354, 925])

        self.ntest_unseen = self.test_unseen_feature.size()[0]
        #print("self.ntest_unseen",self.ntest_unseen)  107859
        self.ntrain_class = self.seenclasses.size(0)
        #print("self.ntrain_class",self.ntrain_class)  925
        self.ntest_class = self.unseenclasses.size(0)
        #print("self.ntest_class",self.ntest_class)  81
        self.train_class = self.seenclasses.clone()       # clone function return a copy of input
        #print("self.train_class",self.train_class.shape)  925
        self.allclasses = torch.arange(0, self.ntrain_class + self.ntest_class).long()
        #print("self.allclasses",self.allclasses.shape)   1006
